...
unsigned int txd_cnt = 0, txd_indx = 0, raw_tx_sent = 0;
			...
			unsigned int ac)
{
	 ...
	 * regular packets.
	 */
	pend_pkt_q = def_dev_ctx->tx_config.data_pending_txq[MAX_PEERS][ac];
				       ...
				       int peer_id)
{
	...
	nrf_wifi_osal_log_dbg("%s: config dump: q=%d, r=%d, rf=%d, len=%d, desc_num=%d",
		__func__,
		config->raw_tx_info.queue_num,
		config->raw_tx_info.rate,
		config->raw_tx_info.rate_flags,
		config->raw_tx_info.pkt_length,
		config->raw_tx_info.desc_num);
					...
					unsigned int ac)
{
	...
	if (_tx_pending_process(fmac_dev_ctx, desc, ac)) {
		first_nwb = nrf_wifi_utils_list_peek(def_dev_ctx->tx_config.pkt_info_p[desc].pkt);
				     ...
				     unsigned char tx_desc_num)
{
...
#ifdef CONFIG_RAW_TX_TRANSMIT_COUNT
		if (tx_sent_complete && ((txd_cnt + def_dev_ctx->raw_pkt_stats.raw_pkt_send_failure) >= raw_tx_sent)) {
			tx_complete = true;
			txd_cnt = 0;
			txd_indx = 0;
		}
	...
	for (frame = 0;
		...
		if (nrf_wifi_osal_nbuf_is_raw_tx((void *)(tx_buf_info->nwb))) {
		     ...
		     struct nrf_wifi_event_raw_tx_done *config)
{
	...
	if (config->status == NRF_WIFI_STATUS_FAIL) {
		def_dev_ctx->raw_pkt_stats.raw_pkt_txd_failure += 1;
		/**
		...
		nrf_wifi_osal_log_dbg("%s: Raw TX failed",
				      __func__);
	}
